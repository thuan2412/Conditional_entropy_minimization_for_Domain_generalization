...
from network import MLP, cmnist_cnn
...
def train(n_steps, envs, model, optimizer, device, l2_weight, p_weight, penalty_anneal_iters, args, method='erm',
    ...
    for step in range(start_step, start_step + n_steps):
        ...
        yy= env['labels'][val_size:]
        if args.ib_lambda > 0.:
            ib_weight = args.ib_lambda if step >= args.ib_step else 0.
            entropy_loss = torch.stack([envs[0]['var'], envs[1]['var']]).mean()
            class_loss = F.cross_entropy(logits[val_size:],yy).div(math.log(2))
